...
with tab1:
    ...
    with query_tabs[0]:
        user_query_custom = st.text_area(
            "Escribe tu consulta",
            placeholder="Ejemplo: Analiza las tendencias de marketing B2B industrial para 2026 y genera 3 recomendaciones estratégicas priorizadas con ROI estimado",
            height=150,
    ...
    with col_openai:
        ...
        selected_openai = st.selectbox("🤖 Modelo OpenAI", list(openai_models.keys()), index=0)
    ...
    with col_perplexity:
        ...
        selected_perplexity = st.selectbox("🔍 Modelo Perplexity", list(perplexity_models.keys()), index=0)
...
Enfoque: Resultados medibles, oportunidades concretas, ROI."""
        ...
        # FASE 2: Perplexity
        with st.spinner(f"🔍 {selected_perplexity} validando con fuentes online..."):
            try:
                from openai import OpenAI
...
Valida con fuentes actuales online y proporciona URLs verificables."""
                ...
                st.success("✅ Contenido generado y validado correctamente")
                st.rerun()
                
            except Exception as e:
                st.error(f"❌ Error en Perplexity: {str(e)}")
                # Usar solo OpenAI como fallback
                if "openai_response" in st.session_state:
                    st.warning("⚠️ Usando solo resultado de OpenAI")
    ...
    if "perplexity_response" in st.session_state:
        ...
        with st.expander("👁️ Vista Previa del Contenido", expanded=True):
            ...
            # Dos columnas para puntos y acciones

...
- Incluye pestaña de Sugerencias con IA
...
# ============================================================================
# NUEVA FUNCIONALIDAD: SUGERENCIAS CON IA
# ============================================================================
def get_url_suggestions(api_key: str, current_results: Dict) -> List[Dict]:
    """
    Usa OpenAI para sugerir nuevas URLs basadas en los resultados actuales.
    """
    client = OpenAI(api_key=api_key)
    
    summary_text = "Entidades analizadas recientemente:\n"
    count = 0
    for centro, data in current_results.items():
        for ent_type, entities in data.get("entities", {}).items():
            for entity in entities:
                name = entity.get("nombre", "")
                desc = entity.get("descripcion", "")[:50]
                inst = entity.get("institucion", "") or centro
                summary_text += f"- {name} ({inst}): {desc}\n"
                count += 1
                if count > 20: break
        if count > 20: break
    ...
    prompt = f"""
    Eres un experto en Dealflow Healthtech en España.
    Basado en las siguientes entidades ya analizadas:
    {summary_text}
    
    Sugiere 5-10 NUEVAS URLs relevantes para analizar. Prioriza:
    1. Perfiles de LinkedIn de las personas clave mencionadas.
    2. Páginas de grupos de investigación de las universidades mencionadas.
    3. Webs de empresas competidoras o relacionadas en el mismo nicho.
    4. Páginas de transferencia de tecnología de los centros mencionados.
    
    Devuelve una lista JSON con objetos que tengan:
    - 'url': La URL sugerida (asegúrate de que sea válida y completa).
    - 'reason': Razón breve de la sugerencia (por qué es relevante).
    
    NO repitas las entidades ya analizadas.
    """
...
def main():
    ...
    if "suggested_urls" not in st.session_state:
        st.session_state.suggested_urls = []
        ...
        if st.button(" Iniciar análisis", type="primary", use_container_width=True):
            if not selected_centros:
                ...
                pipeline = DealflowPipeline(
                    ...
                    orcid_api_key=st.secrets.get("ORCID_API_KEY") if st.secrets.get("ORCID_API_KEY") else None
                )
    ...
    # ------------------------------------------------------------------------
    # TAB 5: SUGERENCIAS CON IA (NUEVA)
    # ------------------------------------------------------------------------
    with tab5:
        st.markdown('<p class="section-title">💡 Sugerencias de Nuevas URLs</p>', unsafe_allow_html=True)
        st.info("Esta herramienta usa IA para analizar tus resultados actuales y sugerir nuevas URLs relevantes (perfiles, grupos, empresas) para re-analizar.")
        
        if not st.session_state.results:
            st.warning("⚠️ Necesitas tener resultados en la pestaña 'Resultados' para generar sugerencias.")
        else:
            col_gen, col_clear = st.columns([3, 1])
            with col_gen:
                if st.button(" Generar Sugerencias con IA", type="primary"):
                    with st.spinner("Analizando resultados y generando sugerencias..."):
                        suggestions = get_url_suggestions(st.session_state.api_key, st.session_state.results)
                        st.session_state.suggested_urls = suggestions
                        st.success(f"✅ Se generaron {len(suggestions)} sugerencias.")
                        st.rerun()
            ...
            if st.session_state.suggested_urls:
                st.markdown("---")
                st.markdown("### Selecciona URLs para analizar:")
                
                selected_urls = []
                for i, sugg in enumerate(st.session_state.suggested_urls):
                    url = sugg.get("url", "")
                    reason = sugg.get("reason", "")
                    
                    with st.container():
                        cb = st.checkbox(f"**{url}**", help=reason)
                        if cb:
                ...
                st.divider()
                if selected_urls:
                    st.markdown(f"**{len(selected_urls)} URLs seleccionadas**")
                    if st.button("🚀 Analizar URLs Seleccionadas", type="primary"):
                        progress_bar = st.progress(0)
                        status_text = st.empty()
                        ...
                        # Crear un centro dummy para las sugerencias
                        dummy_centro = {
                            "nombre": "Sugerencias IA",
                            "region": "Varias",
                            "tipo": "Sugerencias",
                            "urls": selected_urls
                        }
                        
                        status_text.text("🔄 Analizando sugerencias...")
                        result = pipeline.process_center(
                            centro=dummy_centro,
                            tematicas=st.session_state.tematicas_list,
                            max_pages=1 # Solo la URL sugerida
                        )
                        
                        # Añadir resultados al estado global
                        if "Sugerencias IA" not in st.session_state.results:
                            st.session_state.results["Sugerencias IA"] = result
                        else:
                            # Merge entities
                            for et in pipeline.EXTRACTION_ORDER:
                                st.session_state.results["Sugerencias IA"]["entities"][et].extend(result["entities"][et])
                        
                        status_text.text("✅ Análisis de sugerencias completado")
                        st.balloons()
                        st.rerun()
                else:
                    st.warning("Selecciona al menos una URL para analizar.")
            else:
                st.info("Pulsa 'Generar Sugerencias con IA' para empezar.")

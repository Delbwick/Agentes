  ...
  "entities": [
    ...
    def extract_entities(self, content: str, entity_type: str, context: Dict = None) -> Dict:
        ...
        if not prompt_template:
            return {"entities": [], "error": f"Tipo no soportado: {entity_type}", "tokens_used": 0}
        ...
        try:
            ...
            result["tokens_used"] = resp.usage.total_tokens if hasattr(resp, 'usage') else 0
            return result
        except Exception as e:
            return {"entities": [], "error": str(e)[:100], "entity_type": entity_type, "tokens_used": 0}
...
class DealflowPipeline:
    ...
    def __init__(self, api_key: str, orcid_api_key: str = None, model: str = "gpt-4o"):
        self.crawler = WebCrawler()
        self.extractor = EntityExtractor(api_key, model)
    ...
    def process_center(self, centro: Dict, tematicas: List, max_pages: int = 3) -> Dict:
        ...
        for url in centro["urls"][:max_pages]:
            ...
            if cached:
                ...
                results["tokens_used"] += cached.get("tokens_used", 0)
                continue
            ...
            for entity_type in self.EXTRACTION_ORDER:
                ...
                results["tokens_used"] += extracted.get("tokens_used", 0)
            ...
            cache_data["tokens_used"] = results["tokens_used"]
...
def main():
    ...
    with st.sidebar:
        ...
        selected_model = st.selectbox("Modelo de IA", model_options, index=0, help="Selecciona el modelo a usar. Se recomienda usar los modelos más avanzados primero.")
        
        # Token Counter Display
        st.sidebar.metric("Tokens Consumidos", f"{st.session_state.total_tokens:,}")
    ...
    with tab4:
        ...
        if not st.session_state.results:
            ...
            with pd.ExcelWriter(buffer, engine="xlsxwriter") as writer:
                # Sheet 1: Resumen (URLs analizadas por centro)
                summary_rows = []
                for centro_nombre, centro_data in st.session_state.results.items():
                    summary_rows.append({
                        "Centro": centro_nombre,
                        "Región": centro_data.get("region", ""),
                        "URLs Analizadas": len(centro_data.get("urls_analizadas", [])),
                        "Total Entidades": sum(len(e) for e in centro_data.get("entities", {}).values()),
                        "Tokens Consumidos": centro_data.get("tokens_used", 0)
                    })
                if summary_rows:
                    pd.DataFrame(summary_rows).to_excel(writer, sheet_name="Resumen_URLs", index=False)
                
                # Sheets per center
                for centro_nombre, centro_data in st.session_state.results.items():
                    rows = []
                    for entity_type, entities in centro_data.get("entities", {}).items():
                        for entity in entities:
                            rows.append({
            ...
            total_entities_display = sum(
                sum(len(r["entities"].get(et, [])) for et in ["technologies", "papers", "companies", "people"])
                for r in st.session_state.results.values()
            )
            col_s1.metric("Total oportunidades", total_entities_display)
            # Estimate average score
            all_scores = []
            for r in st.session_state.results.values():
                for et in ["technologies", "papers", "companies", "people"]:
                    for e in r["entities"].get(et, []):
            ...
            col_s2.metric("Score promedio", f"{avg_score:.1f}")
            col_s3.metric("Centros analizados", len(st.session_state.results))
            col_s4.metric("Tokens Totales", f"{st.session_state.total_tokens:,}")
            
            # Distribution by type
            type_counts = pd.DataFrame([
                {"Tipo Entidad": et, "Count": sum(len(r["entities"].get(et, [])) for r in st.session_state.results.values())}
                for et in ["technologies", "papers", "companies", "people"]
            ])
            st.markdown("#### Distribución por tipo de entidad")
            st.bar_chart(type_counts.set_index("Tipo Entidad"))

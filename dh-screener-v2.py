Aquí tienes el código corregido y mejorado. He solucionado los errores de sintaxis que tenías (espacios en las claves del diccionario `BRANDING` y el bloque `__name__`) y he añadido la referencia visual al pipeline en la barra lateral para que siempre esté visible cómo funciona el proceso.

### ️ Correcciones aplicadas:
1.  **Error de renderizado HTML**: Asegurado `unsafe_allow_html=True` y corregidas las comillas en el HTML de las tarjetas.
2.  **Error de sintaxis**: Eliminados los espacios al final de las claves en `BRANDING` (ej: `"logo_url "` → `"logo_url"`) que causaban que el logo no cargara.
3.  **Referencia al Pipeline**: Añadido un expander informativo en la sidebar explicando el flujo de trabajo.
4.  **Bloque de ejecución**: Corregido `if name == "main":` a `if __name__ == "__main__":`.

### 📄 Código Completo (`dh-screener-v2.py`)

```python
"""
Double Helix Dealflow Finder v3.0
Pipeline completo para identificar oportunidades de inversión en healthtech:
1. Crawling inteligente de URLs (extrae enlaces internos, limpia tracking)
2. Detección de tipo de página (Spin-offs, Patentes, Startups, etc.)
3. Extracción jerárquica con IA (Tecnologías → Artículos → Empresas → Personas)
4. Enriquecimiento automático de datos (ORCID integration)
5. Caché robusto para evitar re-procesamiento
"""
...
# ⚠️ CORRECCIÓN: Eliminados espacios al final de las claves que causaban error
BRANDING = {
...
st.set_page_config(
    ...
    page_icon="🔍", # ⚠️ CORRECCIÓN: Favicon actualizado
    layout="wide",
...
<style>
    ...
    .logo-subtitle {{ font-size: 0.9rem; color: var(--dh-primary); margin: 0; font-weight: 600; }}
    .stButton>button {{
    ...
    .opportunity-card {{
        ...
        border-radius: 8px; margin: 0.8rem 0; background: #fcfcfc;
    ...
    def lookup_orcid(self, orcid_id: str) -> Optional[Dict]:
        try:
            ...
            if resp.status_code == 200:
                data = resp.json()
                return {
                    "name": self._extract_name(data), "affiliations": self._extract_affiliations(data),
                    "works": self._extract_works(data), "keywords": self._extract_keywords(data), "orcid": orcid_id,
                }
            return None
        except:
            return None
        ...
        try:
            for emp in data.get("activities-summary", {}).get("employments", {}).get("affiliation-group", []):
                for summary in emp.get("summaries", []):
                    ...
                    if org: affiliations.append(org)
        except: pass
    ...
    def _extract_keywords(self, data: Dict) -> List[str]:
        ...
        try:
            for kw in data.get("person", {}).get("keywords", {}).get("keyword", []):
                if kw.get("content"): keywords.append(kw["content"])
        except: pass
        return keywords[:20]

# ============================================================================
# CLASE: ENTITY EXTRACTOR
# ============================================================================
...
CRITERIOS: Tecnologías con aplicación en salud/diagnóstico/farma/biotech, Patentes o invenciones con potencial comercial.
FORMATO JSON:
{{
 "entities": [
{{
 "nombre": "...", "tipo": "tecnología|patente|plataforma|dispositivo",
 "descripcion": "...", "aplicacion_health": "...",
 "madurez": "investigación|prototipo|validación|comercial",
 "score": 0-100, "referencia": "URL o sección", "keywords": ["..."]
}}
],
 "resumen": "Breve descripción del foco tecnológico del centro"
}}""",
        "papers": """Eres un analista científico para Double Helix. Extrae ARTÍCULOS CIENTÍFICOS o PUBLICACIONES con relevancia para healthtech.
FORMATO JSON:
{{
 "entities": [
{{
 ...
 "score": 0-100, "autores_principales": ["..."], "referencia": "DOI o URL"
}}
]
}}""",
        "companies": """Eres un analista de dealflow para Double Helix. Extrae EMPRESAS, STARTUPS o PROYECTOS con potencial de inversión.
FORMATO JSON:
{{
 "entities": [
{{
 "nombre": "...", "tipo": "startup|spin-off|scale-up|proyecto",
 "sector": "diagnóstico|terapias|digital health|biofarma|otros",
 "descripcion": "...", "estado": "seed|series A|growth|exit",
 "equipo": "breve descripción del equipo", "score": 0-100,
 "referencia": "URL", "notas": "Observaciones adicionales"
}}
]
}}""",
        "people": """Eres un analista de talento para Double Helix. Extrae PERSONAS CLAVE (investigadores, founders, CEOs) relevantes.
FORMATO JSON:
{{
 "entities": [
{{
 "nombre": "...", "rol": "investigador|founder|CEO|CTO|advisor",
 "afiliacion": "...", "expertise": ["..."],
 "relevancia": "alta|media|baja", "score": 0-100,
 "contacto": "email/linkedin si está disponible",
 "referencia": "URL", "orcid": "ORCID ID si está disponible"
}}
]
}}"""
    }
    ...
    def extract_entities(self, content: str, entity_type: str, context: Dict = None) -> Dict:
        ...
        context_text = ""
        if context:
            if context.get("centro"): context_text += f"CENTRO: {context['centro']}\n"
            if context.get("region"): context_text += f"REGIÓN: {context['region']}\n"
            if context.get("tematicas"):
                context_text += f"TEMÁTICAS OBJETIVO:\n" + "\n".join(
                    f"- {t['segmento']}: {t['definicion'][:150]}" for t in context["tematicas"][:5]
                ) + "\n"
            if context.get("page_type"): context_text += f"TIPO DE PÁGINA: {context['page_type']}\n"
...
class EuropeanPortalMonitor:
    ...
    def monitor_portals(self, last_check: datetime) -> Dict:
        ...
        cordis_updates = self.check_cordis_updates(
            topics=["digital health", "biotech", "medical devices", "diagnostics"], days_back=7
        )
...
class DealflowPipeline:
    ...
    def process_center(self, centro: Dict, tematicas: List, max_pages: int = 3) -> Dict:
        results = {
            "centro": centro["nombre"], "region": centro.get("region", ""),
            "tipo": centro.get("tipo", ""), "urls_analizadas": [],
            "entities": defaultdict(list), "summary": "", "page_types_found": [],
        }
        for url in centro["urls"][:max_pages]:
            ...
            if cached:
                results["urls_analizadas"].append({
                    "url": url_normalized, "status": "cached",
            ...
            for entity_type in self.EXTRACTION_ORDER:
                extracted = self.extractor.extract_entities(content=page["text"], entity_type=entity_type, context=context)
            ...
            results["urls_analizadas"].append({
                "url": url_normalized, "status": "processed",
                "page_type": page_type,
    ...
    def _enrich_with_orcid(self, people: List[Dict], page_content: str) -> List[Dict]:
        ...
        for person in people:
            orcid_match = re.search(r'(?:orcid\.org/)?(\d{4}-\d{4}-\d{4}-\d{3}[0-9X])', page_content + " " + person.get("referencia", ""), re.I)
...
def load_excel_files(uploaded_files: List) -> tuple:
    ...
    for uploaded_file in uploaded_files:
        try:
            ...
            if "ENLACES" in sheet_names: centros_df = pd.read_excel(xls, sheet_name="ENLACES")
            elif "Sheet1" in sheet_names: centros_df = pd.read_excel(xls, sheet_name=sheet_names[0])
            if "TEMÁTICAS" in sheet_names: tematicas_df = pd.read_excel(xls, sheet_name="TEMÁTICAS")
            elif len(sheet_names) > 1: tematicas_df = pd.read_excel(xls, sheet_name=sheet_names[1])
        except Exception as e:
...
def prepare_tematicas(tematicas_df: pd.DataFrame) -> List[Dict]:
    if tematicas_df is None or tematicas_df.empty: return []
    tematicas = []
    for _, row in tematicas_df.iterrows():
        tematicas.append({
            "vertical": str(row.get("Vertical", "")), "segmento": str(row.get("Segmento", "")),
...
def prepare_centros(centros_df: pd.DataFrame) -> List[Dict]:
    if centros_df is None or centros_df.empty: return []
    centros = []
    for _, row in centros_df.iterrows():
        nombre = str(row.get("NOMBRE", ""))
        if not nombre or nombre == "nan" or pd.isna(nombre): continue
        urls = []
        for col in centros_df.columns:
            col_upper = str(col).upper()
            if col_upper.startswith("WEB") and pd.notna(row.get(col)):
                url = str(row.get(col)).strip()
                if url and url.lower().startswith("http"): urls.append(normalize_url(url))
        if urls:
            centros.append({
...
def render_entity_card(entity: Dict, entity_type: str):
    ...
    if entity_type == "technologies":
        ...
        if entity.get("tipo"): tags_html += f'<span class="entity-tag">{entity["tipo"].upper()}</span>'
    elif entity_type == "papers":
        tags_html += '<span class="entity-tag">📄 Artículo</span>'
        if entity.get("journal"): tags_html += f'<span class="entity-tag">{entity["journal"]}</span>'
    elif entity_type == "companies":
        tags_html += '<span class="entity-tag"> Empresa</span>'
        if entity.get("tipo"): tags_html += f'<span class="entity-tag">{entity["tipo"].upper()}</span>'
    elif entity_type == "people":
        tags_html += '<span class="entity-tag">👤 Investigador</span>'
        if entity.get("rol"): tags_html += f'<span class="entity-tag">{entity["rol"].upper()}</span>'
        if entity.get("orcid"): tags_html += '<span class="entity-tag">ORCID</span>'
    ...
    ref = entity.get("referencia", "")
    
    # ⚠️ CORRECCIÓN: HTML mejorado para asegurar renderizado correcto
    html_content = f"""
    <div class="opportunity-card">
        <div style="display: flex; justify-content: space-between; align-items: start; gap: 1rem;">
            ...
            <div style="text-align: right; min-width: 80px;">
                ...
                {f'<br><a href="{ref}" target="_blank" style="font-size: 0.8rem; color: {BRANDING["primary_color"]}; text-decoration: none; margin-top: 0.5rem; display: inline-block;">🔗 Ver referencia</a>' if ref else ''}
            </div>
        </div>
    </div>
    """
...
# ============================================================================
def main():
    if "centros_df" not in st.session_state: st.session_state.centros_df = None
    if "tematicas_df" not in st.session_state: st.session_state.tematicas_df = None
    if "centros_list" not in st.session_state: st.session_state.centros_list = []
    if "tematicas_list" not in st.session_state: st.session_state.tematicas_list = []
    if "results" not in st.session_state: st.session_state.results = {}
    if "eu_updates" not in st.session_state: st.session_state.eu_updates = None
    if "openai_ok" not in st.session_state: st.session_state.openai_ok = False
    if "api_key" not in st.session_state: st.session_state.api_key = None
    if "last_eu_check" not in st.session_state: st.session_state.last_eu_check = None

    with st.sidebar:
        st.image(BRANDING["logo_url"], width=60)
        st.markdown("### ⚙️ Configuración")
        
        # ⭐ NUEVO: Referencia visual al Pipeline
        with st.expander("ℹ️ Pipeline de Procesamiento", expanded=True):
            st.markdown("""
            **Flujo de trabajo automático:**
            1. ️ **Crawling**: Descarga y limpieza de URLs (elimina tracking UTM).
            2. 🔍 **Detección**: Clasificación inteligente de contenido (Spin-offs, Patentes, Startups, etc.).
            3. 🧠 **Extracción IA**: LLM extrae entidades estructuradas (JSON).
            4. 🆔 **Enriquecimiento**: Búsqueda automática de ORCID para autores.
            5. 💾 **Cache**: Almacenamiento para evitar re-procesamiento (7 días).
            """)

        st.markdown("#### 🤖 OpenAI API")
        api_from_secrets = ""
        try: api_from_secrets = st.secrets.get("OPENAI_API_KEY", "")
        except: pass
        ...
        if uploaded_files:
            if st.button("🔄 Procesar archivos"):
                with st.spinner("Cargando datos..."):
                    ...
                    else: st.error("❌ No se pudo cargar la hoja de centros")
                    ...
                    else: st.warning("⚠️ No se pudieron cargar temáticas")
    ...
    with tab1:
        ...
        if region_filter != "Todas": centros_filtrados = [c for c in centros_filtrados if c["region"] == region_filter]
        if tipo_filter != "Todos": centros_filtrados = [c for c in centros_filtrados if c["tipo"] == tipo_filter]
        ...
        selected_centros = st.multiselect(
            ...
            options=list(centro_options.keys()), default=list(centro_options.keys())[:3]
        )
        ...
        if st.button("🚀 Iniciar análisis", type="primary", use_container_width=True):
            if not selected_centros: st.warning("⚠️ Selecciona al menos un centro")
            else:
                ...
                for idx, centro_key in enumerate(selected_centros):
                    ...
                    result = pipeline.process_center(
                        centro=centro, tematicas=st.session_state.tematicas_list, max_pages=max_pages
                    )
                    for et in pipeline.EXTRACTION_ORDER:
                        result["entities"][et] = [
                            e for e in result["entities"][et] if e.get("score", 0) >= min_score
                        ]
    ...
    with tab2:
        ...
        if not st.session_state.results:
            ...
            with col_f3: score_min = st.slider("Score mínimo", 60, 100, 60)
            with col_f4:
            ...
            for centro_nombre, centro_data in st.session_state.results.items():
                if region_filter and centro_data["region"] not in region_filter: continue
                total_opp_centro = sum(len(centro_data["entities"].get(et, [])) for et in entity_filter)
                if total_opp_centro == 0: continue
                with st.expander(f" {centro_nombre} ({centro_data['region']}) - {total_opp_centro} oportunidades", expanded=True):
                    ...
                    for entity_type in entity_filter:
                        ...
                        for entity in entities:
                            if entity.get("score", 0) < score_min: continue
                            if vertical_filter:
                                ent_vertical = entity.get("vertical") or entity.get("sector")
                                if ent_vertical and ent_vertical not in vertical_filter: continue
    ...
    with tab3:
        ...
        **Temas buscados:** health, biotech, medical, pharma, diagnostic, digital health
        """)
        if not st.session_state.openai_ok: st.warning("⚠️ Configura OpenAI para activar el monitoreo")
        else:
            ...
            with col_info:
                ...
                else: st.caption("Sin consultas recientes")
        if st.session_state.eu_updates:
            ...
            for opp in st.session_state.eu_updates.get("new_opportunities", []):
                with st.container():
                    ...
                    with col2:
                        ...
                        if opp.get("url"): st.markdown(f"[🔗 Ver proyecto]({opp['url']})", unsafe_allow_html=True)
                    st.divider()
        else: st.info("👉 Pulsa 'Consultar actualizaciones' para buscar nuevas oportunidades")

    with tab4:
        st.markdown('<p class="section-title">📊 Exportar resultados</p>', unsafe_allow_html=True)
        if not st.session_state.results: st.info(" Ejecuta un análisis primero para poder exportar")
        else:
            rows = []
            for centro_nombre, centro_data in st.session_state.results.items():
            ...
            if rows:
                ...
                with col_dl2:
                    ...
                    st.download_button(
                        ...
                        file_name=f"double_helix_dealflow_{datetime.now().strftime('%Y%m%d')}.csv", mime="text/csv"
                    )
                st.markdown("#### 📈 Resumen")
            ...
            else: st.warning("⚠️ No hay oportunidades para exportar. Ajusta los filtros o ejecuta un nuevo análisis.")
...
# ⚠️ CORRECCIÓN: Sintaxis correcta para bloque principal

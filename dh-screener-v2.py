...
# ============================================================================
# CLASE: SUGGESTION GENERATOR (NUEVO)
# ============================================================================
class SuggestionGenerator:
    """Genera sugerencias estratégicas basadas en el dealflow analizado."""
    PROMPT_SUGGESTIONS = """Eres un experto en scouting tecnológico y venture capital en healthtech.
Basado en el siguiente análisis de dealflow realizado (tecnologías identificadas, empresas, patentes y keywords extraídas de centros de investigación españoles):
{analyzed_context}
...
5. Breve recomendación general de estrategia de scouting.
...
{{
  ...
  "recomendacion_general": "Texto breve y estratégico..."
    ...
    def generate(self, analyzed_context: str) -> Dict:
        try:
            prompt = self.PROMPT_SUGGESTIONS.format(analyzed_context=analyzed_context)
            resp = self.client.chat.completions.create(
                model="gpt-4o-mini",
...
def main():
    ...
    # ========================================================================
    # PESTAÑAS PRINCIPALES (6 pestañas ahora)
    # ========================================================================
    ...
    with tab3:
        ...
        st.info("Esta sección analiza el dealflow ya extraído y genera recomendaciones para la siguiente ronda de scouting.")
        ...
        if not st.session_state.results:
            ...
            if st.button("🔄 Generar Sugerencias", type="primary"):
                with st.spinner("Analizando dealflow y generando sugerencias..."):
                    ...
                    analyzed_context = "\n".join(context_parts)
                    generator = SuggestionGenerator(api_key=st.session_state.api_key)
                    st.session_state.suggestions = generator.generate(analyzed_context)
            
            if st.session_state.suggestions and "error" not in st.session_state.suggestions:
                ...
                with col_dl1:
                    ...
                    with pd.ExcelWriter(buffer, engine="xlsxwriter") as writer:
                        ...
                        for i, col in enumerate(df_export.columns):
                            max_len = max(df_export[col].fillna('').astype(str).str.len().max(), len(col))
    ...
    # ------------------------------------------------------------------------
    with tab6:
        st.markdown('<p class="section-title">📝 Prompts Utilizados</p>', unsafe_allow_html=True)
        st.info("Aquí puedes revisar y copiar los prompts que utiliza la IA para la extracción y generación de sugerencias.")
        
        st.subheader("🔍 Prompts de Extracción de Entidades")
        for etype, prompt in EntityExtractor.PROMPTS.items():
            with st.expander(f" {etype.upper()}"):
                st.code(prompt, language="text")
                if st.button(f"Copiar {etype}", key=f"copy_{etype}"):
                    st.clipboard = prompt
                    st.toast(f"Prompt de {etype} copiado!")

        st.subheader(" Prompt de Generación de Sugerencias")
        with st.expander(" SuggestionGenerator"):
            st.code(SuggestionGenerator.PROMPT_SUGGESTIONS, language="text")
            if st.button("Copiar Sugerencias", key="copy_sugg"):
                st.clipboard = SuggestionGenerator.PROMPT_SUGGESTIONS
                st.toast("Prompt de sugerencias copiado!")

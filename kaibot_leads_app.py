...
with tab2:
    ...
    st.caption("Haz clic en cualquier celda editable para modificar. Los cambios se guardan automáticamente.")
    
    # Columnas editables
    editable_cols = ["ANOTACIONES", "CARGO", "VERTICAL_EMPRESA", "SON_CLIENTE", "ORIGEN_FORM_HA_FINALIZADO", "VALOR_LEAD", "COSTE_DEL_LEAD"]
    ...
    edited_df = st.data_editor(
        df_f[["N_FORM", "FECHA_ENVIO_FORM", "NOMBRE_EMPRESA", "MAIL", "CARGO", "VERTICAL_EMPRESA", "FACTURACION", "SON_CLIENTE", "ORIGEN_FORM_HA_FINALIZADO", "VALOR_LEAD", "COSTE_DEL_LEAD", "ANOTACIONES", "PUNTUACION", "ESTADO_VALOR", "ROI_LEAD"]],
                ...
                if n_form in st.session_state.leads_df["N_FORM"].values:
                    ...
                    for col in editable_cols:
                        if col in row and col in st.session_state.leads_df.columns:
                            st.session_state.leads_df.at[global_idx, col] = row[col]
            # Recalcular valoraciones
            st.session_state.leads_df = calcular_valoracion(st.session_state.leads_df)
            st.success("✅ Cambios guardados y puntuaciones actualizadas")
            st.rerun()

# TAB 3: Detalle por Empresa (SELECCIÓN POR NOMBRE_EMPRESA)
with tab3:
    ...
    # Selector por empresa (puede haber duplicados, mostrar contador)
    empresa_sel = st.selectbox("Selecciona una empresa", options=empresas, index=0 if empresas else None)
    
    if empresa_sel:
        # Mostrar todos los leads de esa empresa (puede haber varios formularios)
        ...
        if len(leads_empresa) > 1:
            ...
            form_sel = st.selectbox("Selecciona el formulario específico", options=leads_empresa["N_FORM"].tolist())

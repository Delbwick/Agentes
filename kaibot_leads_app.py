...
def buscar_contexto_empresa(nombre_empresa, api_key):
    ...
    try:
        ...
        Si no sabes algo, usa "Desconocido"."""
...
def consultar_openai_enriquecido(row, api_key, icp_config=None):
    ...
    Penaliza si B2C, cargo sin poder, mensaje genérico.
...
with tab4:
    ...
    with st.form("new_lead_form"):
        ...
        emp = c1.text_input("NOMBRE_EMPRESA *", placeholder="TechCorp")
        mail = c2.text_input("MAIL *", placeholder="email@empresa.com")
        cargo = c3.text_input("CARGO", placeholder="CTO")
        vert = c1.selectbox("VERTICAL", ["Tecnología", "Industrial", "Salud", "Finanzas"])
        fact = c2.selectbox("FACTURACION", ["<1M€", "1-5M€", "5-20M€", ">20M€"])
        men = c3.text_area("MENSAJE")
        val = c1.number_input("VALOR (€)", value=1000)
        cost = c2.number_input("COSTE (€)", value=50)
        ...
        if st.form_submit_button("💾 Guardar y Analizar"):
            if emp and mail:
                ...
                new_df = pd.DataFrame([{
                    ...
                    "NOMBRE_EMPRESA": emp, "MAIL": mail, "CARGO": cargo,
                    "VERTICAL_EMPRESA": vert, "FACTURACION": fact, "MENSAJE": men,
                    "VALOR_LEAD": val, "COSTE_DEL_LEAD": cost, "SON_CLIENTE": "No",
                    "ORIGEN_FORM_HA_FINALIZADO": "Sí", "ANOTACIONES": "Nuevo"
                }])
...
# TAB 5: Batch Scoring (NUEVO)
with tab5:
    st.markdown("### 🤖 Batch Scoring IA")
    st.caption("Analiza múltiples leads filtrados simultáneamente con OpenAI.")
    ...
    if not st.session_state.openai_key:
        ...
        with col_sel1:
            mode = st.radio("Modo de selección", ["Todos los filtrados", "Seleccionar manualmente"], index=0)
            
        leads_to_process = []
        if mode == "Todos los filtrados":
            leads_to_process = st.session_state.df_filtrado.copy()
            st.info(f"Se procesarán **{len(leads_to_process)}** leads según los filtros actuales.")
        else:
        ...
        if len(leads_to_process) > 0:
            ...
            st.info("No hay leads para procesar.")

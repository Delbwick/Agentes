...
with st.sidebar:
    st.markdown('<div style="display:flex;align-items:center;gap:10px;margin-bottom:12px;"><img src="https://kaibot.es/wp-content/uploads/2020/07/image1.png" width="36"><span style="font-weight:bold;font-size:1.1rem;">KaiBot Leads</span></div>', unsafe_allow_html=True)
    st.markdown("---")
    ...
    if uploaded:
        try:
            ...
            # Mapeo seguro de columnas si es necesario
...
# 4. MAIN UI
# =============================================================
st.markdown('<div style="display:flex;align-items:center;gap:12px;margin-bottom:8px;"><img src="https://kaibot.es/wp-content/uploads/2020/07/image1.png" width="48"><h1 style="margin:0;font-size:1.8rem;">Panel de Leads & Valoración</h1></div>', unsafe_allow_html=True)
...
with tab1:
    ...
    c1, c2 = st.columns(2)
    with c1:
        st.bar_chart(df_f.groupby("VERTICAL_EMPRESA")["VALOR_LEAD"].sum().rename("Valor por Vertical"))
    with c2:
        st.bar_chart(df_f.groupby("TIPO_FORM")["VALOR_LEAD"].mean().rename("Valor Medio por Formulario"))
...
with tab3:
    ...
    sel_lead = st.selectbox("Selecciona un lead para análisis profundo", options=df_f["N_FORM"].tolist() if len(df_f)>0 else [])
